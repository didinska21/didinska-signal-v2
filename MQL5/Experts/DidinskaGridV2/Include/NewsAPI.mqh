//+------------------------------------------------------------------+
//|                                                    NewsAPI.mqh   |
//|                Modul Koneksi ke NewsAPI.org (berita ekonomi)      |
//|                         Version 2.0 - DAX V2 System               |
//+------------------------------------------------------------------+
// PENJELASAN SINGKAT (bahasa sederhana):
//   File ini TIDAK berhubungan dengan AI Groq — ini modul terpisah yang
//   mengambil berita forex dari NewsAPI.org (gratis, 100 request/hari).
//   Tugasnya:
//     1. FetchForexNews()            -> ambil daftar berita terbaru
//     2. AnalyzeNewsSentiment()      -> hitung skor sentimen (-1 s/d +1)
//     3. IsHighImpactNewsImminent()  -> cek apakah ada berita besar sebentar lagi
//     4. GetNewsSummary()            -> ringkasan berita untuk dikirim ke AI
//
//   Kalau API key belum diisi atau koneksi gagal, modul ini otomatis
//   memakai data contoh (offline mode) supaya EA tetap bisa jalan.
//+------------------------------------------------------------------+
...
public:
   ...
   {
      m_api_key = "";
      m_base_url = "https://newsapi.org/v2/";
      m_last_fetch = 0;
   ...
   double AnalyzeNewsSentiment(string symbol, int hours_ahead = 4)
   {
      ...
      for(int i = 0; i < ArraySize(events); i++)
      {
         if(events[i].event_time >= current_time && events[i].event_time <= future_time)
         {
            ...
            // Simple keyword-based sentiment (enhanced with Groq in main EA)
            string title_lower = events[i].event_title;
            StringToLower(title_lower);
...
private:
   ...
   datetime ParseDateTime(string dt_str)
   {
      ...
      int year = (int)StringToInteger(StringSubstr(dt_str, 0, 4));
      int month = (int)StringToInteger(StringSubstr(dt_str, 5, 2));
      int day = (int)StringToInteger(StringSubstr(dt_str, 8, 2));
      int hour = (int)StringToInteger(StringSubstr(dt_str, 11, 2));
      int min = (int)StringToInteger(StringSubstr(dt_str, 14, 2));
      int sec = (int)StringToInteger(StringSubstr(dt_str, 17, 2));
